 ...
 * the same row and column must be updated to reflect the
 * change
 */
 int updateSudoku(Square *** sudoku, int row, int column)
 {
 ...
 * a solvable square.
 */
 int checkFrame(Square *** sudoku, Box ** boxes)
 {
	 int i, j, x;
 ...
 */
 int ** createFrame()
 {
	...
	frame = (int**)malloc(sizeof(int*)*9);
	
	for(i = 0; i < 9; i++)
	{
		frame[i] = (int*)malloc(sizeof(int)*9);
